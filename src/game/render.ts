...
import { Biome, RESOURCE_COUNT, Resource, Tile, carveTo, isCliffFace } from '../core/tiles.js';
...
function drawWorld(ctx: CanvasRenderingContext2D, state: GameState): void {
  ...
  for (let ty = y0; ty <= y1; ty++) {
    ...
    for (let tx = x0; tx <= x1; tx++) {
      ...
      if (ty + 1 < map.h && shadowLen + 2 <= shadowScratch.length && isCliffFace(tile)) {
        // Face only for Cliff/Steps. A biome seam or a 40-unit grass slope
        // used to draw the same wall and stay walkable — that was the lie.
...
function drawHud(ctx: CanvasRenderingContext2D, state: GameState): void {
  ...
  badgeX = drawInstrumentBadges(ctx, state, badgeX, 4);
...
function drawMiniMap(ctx: CanvasRenderingContext2D, state: GameState): void {
  ...
  const grid =
    state.location.kind === 'dungeon'
      ...
      : state.location.kind === 'interior'
        ? state.exploredInteriors[state.location.interiorId]
  ...
  for (let gi = 0; gi < grid.length; gi++) {
    ...
    for (let py = 0; py < rh; py++) {
      ...
      for (let px = 0; px < rw; px++) {
        ...
        if (pin) {
          if (!state.hasChart && state.location.kind === 'overworld') {
            const g = ((pin[0] + pin[1] + pin[2]) / 3) | 0;
            data[o] = g;
            data[o + 1] = g;
            data[o + 2] = g;
          } else {
          ...
          // Without the Chart, explored land is fog-grey (note 17).
          if (!state.hasChart && state.location.kind === 'overworld') {
            const g = ((data[o] + data[o + 1] + data[o + 2]) / 3) | 0;
            data[o] = g;
            data[o + 1] = g;
            data[o + 2] = g;
          }
...
function drawInstrumentBadges(
  ...
  if (state.hasChart) {
    ...
    ctx.fillRect(cursor, y + 1, 7, 7);
    ctx.fillStyle = '#3a3020';
    ctx.fillRect(cursor + 2, y + 3, 3, 3);
    cursor += 10;
  }
  if (state.hasLodestone) {
    const bearing = lodestoneBearing(state);
    ctx.fillStyle = '#8ab4d8';
    ctx.beginPath();
    ctx.moveTo(cursor + 3, y);
    ctx.lineTo(cursor + 6, y + 8);
    ctx.lineTo(cursor, y + 8);
    ctx.fill();
    if (bearing !== null) {
      ...
      ctx.textAlign = 'left';
      ctx.textBaseline = 'top';
      const label = LODESTONE_DIR_NAME[bearing][0]!.toUpperCase();
      ctx.fillText(label, cursor + 7, y + 1);
      cursor += 14;
    } else {
      cursor += 10;
    }
  ...
  if (state.hasDove) {
    ...
    ctx.fillRect(cursor + 1, y + 2, 5, 4);
    ctx.fillStyle = '#a09080';
    ctx.fillRect(cursor + 5, y + 3, 2, 2);
    cursor += 10;
  }
  return cursor;
}
