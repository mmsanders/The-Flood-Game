...
const TILE_PAINTERS: Record<number, Painter> = {
  ...
  [Tile.WoodSeal]: (c) => {
    base(c, PALETTE.gopher);
    px(c, 0, 0, TILE_PX, 2, '#3a220c');
    px(c, 0, TILE_PX - 2, TILE_PX, 2, '#3a220c');
    c.fillStyle = '#7a4a1e';
    c.fillRect(2, 5, 12, 2);
    c.fillRect(2, 9, 12, 2);
  },
  [Tile.StoneSeal]: (c) => {
    base(c, PALETTE.stoneNode);
    px(c, 0, 0, TILE_PX, 2, '#3a3834');
    px(c, 0, TILE_PX - 2, TILE_PX, 2, '#3a3834');
    c.fillStyle = '#6a6660';
    c.fillRect(3, 4, 4, 3);
    c.fillRect(9, 8, 4, 3);
  },
  [Tile.ReedSeal]: (c) => {
    base(c, PALETTE.reed);
    px(c, 0, 0, TILE_PX, 2, '#2a3a18');
    px(c, 0, TILE_PX - 2, TILE_PX, 2, '#2a3a18');
    c.fillStyle = '#6a8a38';
    c.fillRect(3, 3, 2, 10);
    c.fillRect(7, 4, 2, 9);
    c.fillRect(11, 3, 2, 10);
  },
  [Tile.Water]: (c) => {
